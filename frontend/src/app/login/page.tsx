...
import { useRouter } from "next/navigation"; // Importante: usar next/navigation en lugar de next/router
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Usamos nuestra instancia de Axios configurada
      const response = await api.post("/auth/login", { email, password });

      // Guardamos el token en el navegador
      localStorage.setItem("token", response.data.token);

      // Redirigimos al usuario al tablero principal (tracker de gastos)
      router.push("/");
    } catch (err: any) {
      ...
      <div style={{ width: "100%", maxWidth: "420px", display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        <div style={cardStyle}>
          ...
          <p style={{ marginTop: "24px", textAlign: "center", fontSize: "13px", color: "#64748B" }}>
            ...
            <Link href="/register" style={{ color: "#1D6BF3", fontWeight: 600, textDecoration: "none" }}>
              Regístrate aquí
            </Link>
          </p>
        </div>
