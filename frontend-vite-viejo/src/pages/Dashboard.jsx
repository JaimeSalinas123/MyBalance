...
export default function Dashboard() {
  ...
  const [totales, setTotales] = useState({ balance: 0, ingresos: 0, gastos: 0 });
  const [movimientos, setMovimientos] = useState([]);
  const [mostrarModal, setMostrarModal] = useState(false);
  
  // NUEVO ESTADO: Controla si se muestra la tarjeta de confirmación de eliminación y qué ID borrar
  ...
  const guardarIngreso = async () => {
    ...
    try {
      ...
      alert('Asegúrate de que el backend esté corriendo.');
    }
  };

  // NUEVAS FUNCIONES DE ELIMINACIÓN
  const iniciarEliminacion = (id) => {
    setTransaccionAEliminar(id); // Abre la tarjeta de confirmación
  };

  const cancelarEliminacion = () => {
    setTransaccionAEliminar(null); // Cierra la tarjeta sin hacer nada
  };
  ...
  const confirmarEliminacion = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/transacciones/${transaccionAEliminar}`, {
        method: 'DELETE'
      });

      if (res.ok) {
        cargarBalance(); 
        setTransaccionAEliminar(null); // Cierra la tarjeta después de borrar
      } else {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px 80px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ background: '#0A0F1E', borderRadius: '24px', padding: '40px', position: 'relative', overflow: 'hidden', minHeight: '200px' }}>
              ...
              {movimientos.slice(0, 5).map(mov => (
                <div key={mov.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#F8FAFC', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    {/* Botón actualizado para abrir la nueva tarjeta de confirmación */}
      ...
      {/* ── NUEVO MODAL: Tarjeta de confirmación para eliminar ── */}
